...
public class MentoringRecordController {
    ...
    public ResponseEntity<ApiResponse<MentoringRecordResponseDto>> getMentoringRecord(
        ...
        try {
            ...
            // 해당 멘토링의 참여자만 조회 가능
            if (!mentoringRecord.getSession().isMentorParticipant(userId) && 
                !mentoringRecord.getSession().isMenteeParticipant(userId)) {
    ...
    public ResponseEntity<ApiResponse<Long>> addMenteeFeedback(
        ...
        try {
            ...
            Long mentoringRecordId = mentoringRecordService.createOrUpdateMentoringRecord(sessionId, requestDto);
    ...
    public ResponseEntity<ApiResponse<Long>> uploadConsultationAudio(
        ...
        try {
            ...
            Long mentoringRecordId = mentoringRecordService.uploadAudioAndProcess(sessionId, audioFile);
            return ResponseEntity.ok(ApiResponse.success(mentoringRecordId));
